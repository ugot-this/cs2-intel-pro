...
import { Eye, EyeOff, Crosshair, ArrowRight } from "lucide-react";
import { cn } from "@/lib/utils";

// ─── NextAuth error → readable message ────────────────────────

const AUTH_ERRORS: Record<string, string> = {
  OAuthSignin:         "Google нэвтрэлтийн урсгалд алдаа гарлаа.",
  OAuthCallback:       "Google callback-д алдаа гарлаа. Дахин оролдоно уу.",
  OAuthCreateAccount:  "Google данснаас хэрэглэгч үүсгэхэд алдаа гарлаа.",
  OAuthAccountNotLinked:
    "Энэ имэйл хаяг өөр аргаар бүртгэгдсэн байна. Нууц үгээр нэвтэрнэ үү.",
  AccessDenied:        "Нэвтрэлтийн зөвшөөрөл олгоогүй.",
  Configuration:       "Серверийн тохиргооны алдаа. Дахин оролдоно уу.",
...
export function LoginForm({ googleEnabled }: { googleEnabled: boolean }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (result?.error) {
        setError(AUTH_ERRORS.CredentialsSignin);
      } else {
  ...
  return (
    <div className="rounded-2xl border border-border/60 bg-card overflow-hidden shadow-[0_0_40px_rgba(0,0,0,0.4)]">
      ...
      <div className="px-7 pt-7 pb-5 border-b border-border/40">
        ...
        <h1 className="text-2xl font-black leading-tight">Нэвтрэх</h1>
        <p className="text-muted-foreground/60 text-sm mt-1">
          Өөрийн бүртгэлрүү нэвтэрнэ үү
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <input
              ...
              className={cn(
                ...
                "focus:outline-none focus:border-primary/50 focus:bg-background/60 focus:ring-0",
                "transition-colors"
              )}
            />
          </div>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <Link href="/reset-password" className="text-[11px] text-primary/70 hover:text-primary transition-colors">
                Нууц үг мартсан?
              </Link>
            </div>
          ...
          <button
            ...
            disabled={loading}
            className={cn(
              "w-full flex items-center justify-center gap-2 rounded-xl h-11 text-sm font-bold tracking-wide transition-all",
